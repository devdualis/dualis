...
## DualisCheckUp Multi-Agent Architecture
...
- **Directorio:** `.agents/agents/orchestrator/agent.md`
- **Rol:** Coordinador maestro del flujo de trabajo, ejecución de la batería de pruebas multi-agente (`npm run test:battery`), recolección de métricas y generación de reportes diagnósticos ejecutivos (`.agents/agents/agent-battery-report.json` y `.agents/agents/agent-battery-report.md`).

### 2. Agente Médico Clínico (`doctor_agent`)
- **Directorio:** `.agents/agents/doctor/agent.md`
...
- **Directorio:** `.agents/agents/ai_reviewer/agent.md`
...
- **Directorio:** `.agents/agents/backend/agent.md`
...
- **Directorio:** `.agents/agents/frontend/agent.md`
...
Para ejecutar el ciclo de pruebas completo que evalúa la concordancia médica, latencia, tests de backend y tests de frontend:
```bash
cd backend && npm run test:battery
```
