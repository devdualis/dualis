...
**Domain:** Cross-Platform Mobile Preventive Health Triage & Clinical Analytics Platform  
...
DualisCheckUp combines daily psycho-emotional and physical symptom triage with an anti-tampering temporal audit engine (Antiburla), dual-view visual analytics (2D Anatomical Heat Map and 7-day emotional trends), and an asynchronous biomarker extraction pipeline for laboratory reports.

The architecture demands:
1. **Sub-2-second end-to-end triage latency** with deterministic emergency escalation (Level 4–5 red screen halt).
2. **Strict Brazilian LGPD healthcare data compliance** through Row-Level Security (RLS) and regional data residency (`sa-east-1` / São Paulo).
3. **Decoupled OCR + LLM processing** for multi-page laboratory exams to avoid multimodal token bloat and hallucination while preserving numeric precision.
4. **Compile-time safe, reactive client state management** that automatically cleans up ephemeral triage sessions upon completion.
...
```yaml
name: dualis_checkup
description: "DualisCheckUp Preventive Health & Clinical Triage Mobile App"
publish_to: "none"
version: 1.0.0+1
...
npm install @nestjs/core@12.0.1 @nestjs/common@12.0.1 @nestjs/platform-fastify@12.0.1 @nestjs/config@4.0.0 reflect-metadata@0.2.2 rxjs@7.8.1

# Database Layer (Drizzle ORM + PostgreSQL Driver)
npm install drizzle-orm@0.45.2 pg@8.13.3
npm install -D drizzle-kit@0.30.5 @types/pg@8.11.11

# Redis & Background Queue (BullMQ)
npm install ioredis@6.0.0 bullmq@6.3.4 @nestjs/bullmq@12.0.0

# AI & Document Processing
npm install @google/genai@2.22.0 @google-cloud/vision@6.1.0 @aws-sdk/client-s3@3.750.0 @aws-sdk/s3-request-presigner@3.750.0

# Security, Auth & Utilities
npm install @nestjs/jwt@12.0.0 @nestjs/passport@12.0.0 passport@0.7.0 passport-jwt@4.0.1 argon2@0.41.1
npm install zod@4.6.4 class-validator@0.14.1 class-transformer@0.5.1
npm install helmet@8.0.0 @nestjs/throttler@6.4.0 date-fns@4.1.0 nestjs-pino@4.3.0 pino-http@10.4.0

# Development & Testing Dependencies
npm install -D typescript@5.7.3 @types/node@22.13.0 @types/passport-jwt@4.0.1
npm install -D vitest@3.0.5 supertest@7.0.0 @types/supertest@6.0.2 rimraf@6.0.1
```
...
**If deploying on Google Cloud Platform (GCP):**
...
**If deploying on Amazon Web Services (AWS):**
...
**If mobile client operates in low-connectivity / offline settings:**
...
1. **Package Freshness & Currency:**
   - Flutter packages verified against live `pub.dev` metadata (September 2026):
     - `flutter_riverpod`: `3.4.3` (published Sep 3, 2026)
     - `go_router`: `18.0.1` (published Sep 2, 2026)
     - `dio`: `5.11.1` (published Sep 4, 2026)
     - `fl_chart`: `1.2.0` (published Mar 13, 2026)
     - `flutter_secure_storage`: `11.1.1` (published Sep 11, 2026)
     - `freezed`: `4.0.1` (published Aug 29, 2026)
     - `flutter_svg`: `2.3.0` (published May 8, 2026)
     - `url_launcher`: `6.3.2` (published Jul 10, 2025)
     - `file_picker`: `12.3.0` (published Sep 11, 2026)
     - `image_picker`: `1.2.3` (published Jun 30, 2026)
     - `google_fonts`: `8.2.1` (published Jul 31, 2026)
   - Backend npm packages verified against live npm registry / jsdelivr (September 2026):
     - `@nestjs/core`: `12.0.1`
     - `@nestjs/bullmq`: `12.0.0`
     - `bullmq`: `6.3.4`
     - `ioredis`: `6.0.0`
     - `drizzle-orm`: `0.45.2`
     - `@prisma/client`: `7.10.0` (evaluated as alternative)
     - `@google/genai`: `2.22.0` (official unified SDK)
     - `@google-cloud/vision`: `6.1.0`
     - `zod`: `4.6.4`
2. **Clinical Safety Architecture:**
   - Verified that AI classification latency remains within the <2s budget using Gemini 1.5 Flash structured outputs.
   - Enforced zero-failure deterministic emergency fail-safe using local regex/rules before and alongside LLM classification.
3. **Decoupled OCR + LLM Strategy:**
   - Solves the multi-page lab exam latency and cost trap by decoupling pixel-level layout OCR (Google Cloud Vision) from semantic biomarker extraction (Gemini 1.5 Flash).
4. **LGPD & RLS Security:**
   - Row-Level Security configured at PostgreSQL engine level with Drizzle ORM native parameter pass-through (`SET LOCAL app.current_user_id = $1`).
...
---
*Stack research for: DualisCheckUp Cross-Platform Mobile Preventive Health Platform*  
*Researched: 2026-09-13*
